...
class agentServices {
  // CREATE THE BLOG
  createAgent = async (bodyData: any) => {
    if (!bodyData) return null;
    return await axiosInstance.post(`/agents`, bodyData);
  };

  updateAgent = async (id: string, bodyData: any) => {
    if (!id || !bodyData) return null;
    return await axiosInstance.patch(`/agents/${id}`, bodyData);
  };

  // GET ALL BLOGS
  getAllAgents = async ({ queryKey }: any) => {
  ...
  deleteAgent = async (id: string) => {
    if (!id) return null;
    return await axiosInstance.delete(`/agents/${id}`);
  };
  // BLOG BY USER ID
  //   getAgentsByUserId = async ({ queryKey }: any) => {
