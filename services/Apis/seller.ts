...
class sellerServices {
  // CREATE THE BLOG
  createSeller = async (bodyData: any) => {
    if (!bodyData) return null;
    return await axiosInstance.post(`/sellers`, bodyData);
  };

  updateSeller = async (id: string, bodyData: any) => {
    if (!id || !bodyData) return null;
    return await axiosInstance.patch(`/sellers/${id}`, bodyData);
  };

  // GET ALL BLOGS
  getAllSellers = async ({ queryKey }: any) => {
    ...
    const response = await axiosInstance.get(`/sellers`, { params });
  ...
  // BLOG BY USER ID
  //   getSellersByUserId = async ({ queryKey }: any) => {
