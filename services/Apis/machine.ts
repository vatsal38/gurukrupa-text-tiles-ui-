...
class machineServices {
  // CREATE THE BLOG
  createMachine = async (bodyData: any) => {
    if (!bodyData) return null;
    return await axiosInstance.post(`/machines`, bodyData);
  };

  updateMachine = async (id: string, bodyData: any) => {
    if (!id || !bodyData) return null;
    return await axiosInstance.patch(`/machines/${id}`, bodyData);
  };

  // GET ALL BLOGS
  getAllMachines = async ({ queryKey }: any) => {
  ...
  deleteMachine = async (id: string) => {
    if (!id) return null;
    return await axiosInstance.delete(`/machines/${id}`);
  };
  // BLOG BY USER ID
  //   getMachinesByUserId = async ({ queryKey }: any) => {
